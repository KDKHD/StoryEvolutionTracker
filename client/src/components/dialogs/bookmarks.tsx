...
const Content: React.FC<ContentProps> = ({ resource, updateSearch }) => {
    ...
    return (
      <List>
        <ListItem autoFocus button>
          <Box className={classes.listItem}>{"No Bookmarks"}</Box>
        </ListItem>
      </List>
    );
  }
  ...
  return (
    <List>
      {data.user.bookmarks.map((bookmark: any, i: number) => (
...
const BookmarksDialog: React.FC<Props> = (props) => {
  const classes = useStyles();
  ...
  return (
    <Dialog
      ...
      open={props.open}
    >
      <Box className={classes.title}>{"Bookmarks"}</Box>
      <Suspense
        fallback={
          <List>
            <ListItem autoFocus button>
